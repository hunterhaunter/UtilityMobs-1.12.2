...
public class BlockEvent extends UtilityMobsEvent
{
    ...
    public BlockEvent(EntityPlayer player, int x, int y, int z, int face) {
        this(player, x + EnumFacing.byIndex(face).getXOffset(), y + EnumFacing.byIndex(face).getYOffset(), z + EnumFacing.byIndex(face).getZOffset());
    }

    public BlockEvent(EntityPlayer player, int x, int y, int z) {
        this.entityPlayer = player;
        ItemStack held = player.getHeldItemMainhand();
        this.holdingGolemHead = !held.isEmpty() && (held.getItem() == Item.getItemFromBlock(Blocks.PUMPKIN) || held.getItem() == Item.getItemFromBlock(Blocks.LIT_PUMPKIN));
        this.posX = x;
        this.posY = y;
        this.posZ = z;
    }
    ...
    @Override
    public void execute() {
        BuildHelper.place(this.entityPlayer.world, this.entityPlayer, this.holdingGolemHead, this.posX, this.posY, this.posZ);
    }
}
